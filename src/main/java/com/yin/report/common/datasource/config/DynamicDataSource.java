...
public class DynamicDataSource extends DataSource {
    ...
    private DataSource initDDS(String projectCode) throws IllegalArgumentException, IllegalAccessException {
        ...
        DatasourceConfig datasourceConfig = ProjectDBStorage.instance().getDatasourceConfig(projectCode);
        dds.setUrl(datasourceConfig.getDbUrl());
        dds.setUsername(datasourceConfig.getDbUsername());
        dds.setPassword(datasourceConfig.getDbPassword());
        dds.setDriverClassName(datasourceConfig.getDbDriverClassName());
        return dds;
    }
}
